module collision_tables_mod
    ...
    public :: get_Zr_ref, sample_dissp
  ...
  contains
    ...
    subroutine get_Zr_ref(AR, Zr_ref)
      implicit none
      real(dp), intent(in)  :: AR
      real(dp), intent(out) :: Zr_ref

      ! Reference AR values
      real(dp), parameter :: AR_vals(4) = (/ &
           1.5d0, 2.0d0, 2.5d0, 3.0d0 /)

      ! Corresponding Zr_ref values
      real(dp), parameter :: Zr_vals(4) = (/ &
           4.95d0, 3.48d0, 3.81d0, 4.10d0 /)

      integer :: i

      do i = 1, size(AR_vals)
         if (abs(AR - AR_vals(i)) < 1.0d-12) then
            Zr_ref = Zr_vals(i)
            return
         end if
      end do

      stop "ERROR: AR out of scope in get_Zr_ref"

    end subroutine get_Zr_ref
    ...
    recursive real(dp) function sample_gamma(shape) result(g)
      ...
      if (shape < 1.0_dp) then
          ...
          call random_number(u)
          g = sample_gamma(shape + 1.0_dp) * u**(1.0_dp/shape)
          return
      end if
    ...
    ! Sample dissipation fraction from Beta(a,b)
    real(dp) function sample_dissp(a, b) result(gamma)
