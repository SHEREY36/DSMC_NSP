module gmm_cond_mod
    implicit none
    private
    public :: init_gmm_cond, sample_conditionals, print_gmm_debug, get_gmm_filepath
  ...
  function get_gmm_filepath(AR) result(filepath)
    ! Returns the path to the GMM binary file for a given aspect ratio
    ! Tries DSMC_GMM_DIR environment variable first, falls back to ../collision_models/
    real(8), intent(in) :: AR
    character(len=512) :: filepath
    character(len=32) :: ar_label
    character(len=512) :: gmm_dir
    integer :: stat
    logical :: file_exists

    ! Convert AR to label
    call ar_to_label(AR, ar_label)

    ! Try environment variable first
    call get_environment_variable("DSMC_GMM_DIR", gmm_dir, status=stat)
    if (stat == 0 .and. len_trim(gmm_dir) > 0) then
      filepath = trim(gmm_dir) // "/gmm_cond_AR" // trim(ar_label) // ".bin"
    else
      ! Default: check for symlink in run directory, then relative path
      filepath = "collision_models/gmm_cond_AR" // trim(ar_label) // ".bin"
    ...
    inquire(file=trim(filepath), exist=file_exists)
    if (.not. file_exists) then
      write(*,*) "ERROR: GMM file not found: ", trim(filepath)
      write(*,*) "Looking for AR =", AR
      stop "GMM file missing"
    end if

  end function get_gmm_filepath

  subroutine ar_to_label(AR, label)
    ! Maps AR values to filename suffixes
    real(8), intent(in) :: AR
    ...
    if (abs(AR - 1.1d0) < tol) then
      label = "11"
    else if (abs(AR - 1.25d0) < tol) then
      label = "125"
    else if (abs(AR - 1.5d0) < tol) then
      label = "15"
    else if (abs(AR - 2.0d0) < tol) then
      label = "20"
    else if (abs(AR - 2.5d0) < tol) then
      label = "25"
    else if (abs(AR - 3.0d0) < tol) then
      label = "30"
    else
      write(*,*) "ERROR: Unsupported aspect ratio AR =", AR
      write(*,*) "Supported values: 1.1, 1.25, 1.5, 2.0, 2.5, 3.0"
      stop "Invalid AR value"
    end if

  end subroutine ar_to_label
