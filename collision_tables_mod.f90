module collision_tables_mod
    ...
    ! Model orders (Python: m=0..2, n=0..2, k=0..4)
    integer, parameter :: M = 3   ! m = 0..2  → 3 values
    integer, parameter :: N = 3   ! n = 0..2  → 3 values
    integer, parameter :: K = 5   ! k = 0..4  → 5 values

    real(dp), parameter :: beta = 0.5d0

    ! ------------------ Elastic coefficients (M=3, K=5) ------------------
    ! From Python: a_elastic(m,k) for m=0,1,2 and k=0,1,2,3,4
    real(dp), parameter :: a_elastic(M,K) = reshape((/ &
      ! m=0: k=0,1,2,3,4
      1.6873231093532046d-01, -2.2335201400824332d+00,  2.5972229944444980d+01, &
     -3.8369445192346824d+01,  1.4414950493163628d+01, &
      ! m=1: k=0,1,2,3,4
     -2.3422025542515978d-01,  7.7824615377092705d+00, -2.5437436684757500d+01, &
      2.4542914992197733d+01, -6.5683799197148618d+00, &
      ! m=2: k=0,1,2,3,4
      4.6990593038409800d-02, -9.1796465773107383d-01,  2.3118578511070478d+00, &
     -1.3275712470782079d+00, -1.3361202626719099d-01  &
    /), (/M,K/))

    ! ------------------ Delta-p coefficients (M=3, N=3, K=5) ------------------
    ! From Python: a_dp(m,n,k) for m=0,1,2, n=0,1,2, k=0,1,2,3,4
    real(dp), parameter :: a_dp(M,N,K) = reshape((/ &
      ! m=0, n=0: k=0,1,2,3,4
      6.2414983673001086d-02, -7.3368333108724637d-01,  2.4322928469077385d+00, &
     -3.0492092762828156d+00,  1.2815327955067102d+00, &
      ! m=0, n=1: k=0,1,2,3,4
     -1.2166411325362669d+00,  2.1824966567939089d+01, -2.4758309845418431d+01, &
     -4.9691898664908692d+01,  5.4879389762259606d+01, &
      ! m=0, n=2: k=0,1,2,3,4
      2.6412861315526110d+00, -1.6414556363006179d+00, -6.9964611761633819d+01, &
      1.5677895634098536d+02, -8.8316725032941164d+01, &
      ! m=1, n=0: k=0,1,2,3,4
     -5.0945613314919556d-02,  7.7906830826760654d-01, -2.5784901055570839d+00, &
      2.9615089265599890d+00, -1.0982420739614940d+00, &
      ! m=1, n=1: k=0,1,2,3,4
      6.3399165761527665d-01, -5.5213896773089326d+00, -1.7172350210975793d+01, &
      7.1411515099552773d+01, -5.0011678127184418d+01, &
      ! m=1, n=2: k=0,1,2,3,4
     -2.3392938619784158d+00,  5.5774538557737642d+01, -2.0607989894238793d+02, &
      2.5453928675442623d+02, -1.0252047801167170d+02, &
      ! m=2, n=0: k=0,1,2,3,4
      8.0399264059662010d-03, -1.4628184477457487d-01,  4.8861229976985776d-01, &
     -5.3935769938669287d-01,  1.8545748352060740d-01, &
      ! m=2, n=1: k=0,1,2,3,4
     -4.1559277887395225d-02,  1.0276731605332390d+00,  6.2780114321076874d-01, &
     -7.5408087306280756d+00,  6.0201491869751136d+00, &
      ! m=2, n=2: k=0,1,2,3,4
      1.2668998958113506d-02, -9.2319168509144927d+00,  4.0752937305957317d+01, &
     -5.3978751623663598d+01,  2.2570339021666808d+01  &
    /), (/M,N,K/))
  ...
  contains
    ...
    pure real(dp) function p_chi(chi)
      ...
      ! Elastic contribution: sum over m=0..2, k=0..4
      do m = 1, M
        do k = 1, K
          chi_k = chi**(k-1)
          s = s + a_elastic(m,k) * AR_c**(m-1) * chi_k
        end do
      end do

      ! Delta-p contribution: sum over m=0..2, n=0..2, k=0..4
      do m = 1, M
        do n = 1, N
          do k = 1, K
            chi_k = chi**(k-1)
            s = s + a_dp(m,n,k) * AR_c**(m-1) * phi_c**(n-1) * chi_k
          end do
        end do
      end do
    ...
    subroutine init_pchi(AR, alpha)
      ...
      pchi_max = 0.0d0
      do i = 1, NCHI+1
         chi = real(i-1,dp)/real(NCHI,dp)
         pval = p_chi(chi)
         if (pval > pchi_max) pchi_max = pval
      end do

      pchi_max = 1.05d0*pchi_max   ! safety margin
    ...
    subroutine get_delta_eps_max(alpha, AR, delta_eps_max)
      ...
      ! Dimensions (9,5) to match the data stream order
      ...
      do i = 1, size(alpha_vals)
           if (abs(alpha - alpha_vals(i)) < 1.0d-12) then
               ia = i
               exit
           end if
      end do
